...
parser.add_argument("-i", "--intergenic", help=" Number of intergenic space. Default = 150 ")
...
parser.add_argument("-v", "--version", action="version", version='%(prog)s 1.2.6.0')
...
##TAGs
								...
								if mainPoint+2 in sameStrand or mainPoint+2 in diffStrand:
									...
									if int(gappyness)>int(difference2) or int(gappyness)==int(difference2):
										if int(FamListDict[keys][mainPoint+2])!=0:
											...
											if mainPoint+3 in sameStrand or mainPoint+3 in diffStrand:
												GapStart3=int(posListDict[keys][mainPoint+2].split(':')[1])
												GapEnd3=int(posListDict[keys][mainPoint+3].split(':')[0])
												difference3=(GapEnd3-GapStart3)-1
												if int(gappyness)>int(difference3) or int(gappyness)==int(difference3):
													if int(FamListDict[keys][mainPoint+3])!=0:
														sameStrandWindowD.append(mainPoint+3)
														#print(keys,mainPoint, LengthOfFlankFound, strandListDict[keys],posListDict[keys][mainPoint+2], posListDict[keys][mainPoint+3], difference3)
					if mainPoint-1 in sameStrand or mainPoint-1 in diffStrand:
								...
								if mainPoint-2 in sameStrand or mainPoint-2 in diffStrand:
									...
									if int(gappyness)>int(difference5) or int(gappyness)==int(difference5):
										if int(FamListDict[keys][mainPoint-2])!=0:
											...
											if mainPoint-3 in sameStrand or mainPoint-3 in diffStrand:
												GapStart6=int(posListDict[keys][mainPoint-3].split(':')[1])
												GapEnd6=int(posListDict[keys][mainPoint-2].split(':')[0])
												difference6=(GapEnd6-GapStart6)-1
												if int(gappyness)>int(difference6) or int(gappyness)==int(difference6):
													if int(FamListDict[keys][mainPoint-3])!=0:
														sameStrandWindowU.append(mainPoint-3)
														#print(keys,mainPoint, LengthOfFlankFound, strandListDict[keys],posListDict[keys][mainPoint-3], posListDict[keys][mainPoint-2], difference6, int(FamListDict[keys][mainPoint-3]))
	...
	for keys in OperonFamilyDict:
		operonList=[]
		if len(OperonFamilyDict[keys])>3:
			for i in range (len(OperonFamilyDict[keys])-3):
				for j in range (1,len(OperonFamilyDict[keys])-2):
					for k in range (2,len(OperonFamilyDict[keys])-1):
						for l in range (3,len(OperonFamilyDict[keys])):
							if j==i+1 and k==j+1 and l==k+1:
								...
								or int(OperonFamilyDict[keys][k]) in queryFamilySet or int(OperonFamilyDict[keys][l]) in queryFamilySet:
									probableOperon=str(OperonFamilyDict[keys][i])+'.'+str(OperonFamilyDict[keys][j])+'.'+\
									str(OperonFamilyDict[keys][k])+'.'+str(OperonFamilyDict[keys][l])
	...
	for items in probableTApairFamDict:
		for item in probableTApairFamDict[items]:
			...
			for operons in  operonOccuranceDict:
				...
				operonItem3=operons.split('.')[2:4]
				if taItem==operonItem1 or taItem==operonItem2 or taItem==operonItem3:
